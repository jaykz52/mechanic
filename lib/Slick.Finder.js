...
local.search = function(context, expression, append, first){
  ...
  // else if (!context.nodeType) return found;
	...
	var parsed, i,
		...
		contextIsDocument = (context == app);
    this.setDocument(app);
	...
	if (typeof expression == 'string'){ // expression is a string
		...
		simpleSelectors: if (simpleSelector) {
			...
			if (!symbol){
				...
				node = context.getElementsById(name);
...
local.matchSelector = function(node, tag, id, classes, attributes, pseudos){
  ...
  // }
	...
	if (id && node.getAttribute('name') != id) return false;
...
var combinators = {
	...
	' ': function(node, tag, id, classes, attributes, pseudos, classList){ // all child nodes, any level
		...
		var i, item, items, children;

		if (this.isHTMLDocument){
			getById: if (id){
				items = node.getElementsById(id);
        var i;
        for (i = 0; i < items.length; i++) {
          this.push(items[i], tag, null, classes, attributes, pseudos);
        }
        return;
